    ...
    <div>
      <section className="custom-hero">
        <div className="custom-hero-text">
          <span className="custom-badge">Maison de Beaute Exclusive</span>
          <h1>Gold Infused Rejuvenators</h1>
          <p>Custom formulated anti-aging oils, premium foundation palettes, and luxury french perfumes.</p>
          <button className="custom-btn" onClick={() => state.navigate('category')}>Shop Now ✨</button>
        </div>
        <div className="custom-hero-visual">
          <img src="https://placehold.co/500x400/e1bee7/9c27b0?text=Luxe%20Foundations" alt="Hero Visual" className="custom-hero-img" />
        </div>
      </section>
      ...
      <section className="custom-section">
        ...
        <div className="custom-category-grid">
          {CATEGORIES.slice(0, 4).map(cat => (
            <div key={cat.id || cat.name} className="custom-category-card" onClick={() => { state.setSelectedCategory(cat.name); state.navigate('category'); }}>
              <div className="custom-category-icon" style={{ background: '#e1bee7' }}>{cat.icon || '📦'}</div>
              <h3>{cat.name}</h3>
            </div>
          ))}
        </div>
