    ...
    <div>
      <section className="custom-hero">
        <div className="custom-hero-text">
          <span className="custom-badge">ColorStudio Pro Exclusive</span>
          <h1>Infinite Color Palettes</h1>
          <p>Professional liquid eyeshadows, highlighters, waterproof contour kits, and custom brush rolls.</p>
          <button className="custom-btn" onClick={() => state.navigate('category')}>Shop Now ✨</button>
        </div>
        <div className="custom-hero-visual">
          <img src="https://placehold.co/500x400/f8bbd0/e91e63?text=Makeup%20Palettes" alt="Hero Visual" className="custom-hero-img" />
        </div>
      </section>
        ...
        <div className="custom-category-grid">
          {CATEGORIES.slice(0, 4).map(cat => (
            <div key={cat.id || cat.name} className="custom-category-card" onClick={() => { state.setSelectedCategory(cat.name); state.navigate('category'); }}>
              <div className="custom-category-icon" style={{ background: '#f8bbd0' }}>{cat.icon || '📦'}</div>
              <h3>{cat.name}</h3>
            </div>
          ))}
        </div>
