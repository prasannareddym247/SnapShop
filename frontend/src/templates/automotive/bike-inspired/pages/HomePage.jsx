    ...
    <div>
      <section className="custom-hero">
        <div className="custom-hero-text">
          <span className="custom-badge">MotoGear Rider Exclusive</span>
          <h1>Full-Face DOT Helmets</h1>
          <p>Carbon knuckle leather riding gloves, custom slip-on exhausts, and high-tensile sprocket chain kits.</p>
          <button className="custom-btn" onClick={() => state.navigate('category')}>Shop Now ✨</button>
        </div>
        <div className="custom-hero-visual">
          <img src="https://placehold.co/500x400/fef3c7/f59e0b?text=Moto%20Accessories" alt="Hero Visual" className="custom-hero-img" />
        </div>
      </section>
        ...
        <div className="custom-category-grid">
          {demoCategories.slice(0, 4).map(cat => (
            <div key={cat.id} className="custom-category-card" onClick={() => { state.setSelectedCategory(cat.name); state.navigate('category'); }}>
              <div className="custom-category-icon" style={{ background: '#fef3c7' }}>{cat.icon}</div>
              <h3>{cat.name}</h3>
            </div>
          ))}
        </div>
