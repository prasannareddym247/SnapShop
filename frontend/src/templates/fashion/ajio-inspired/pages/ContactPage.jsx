  ...
  return (
    <div className="custom-page-container">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', marginTop: '1.5rem' }}>
        ...
        <div>
          <h3>Merchant Info</h3>
           <p>📞 Phone: {state.store?.phone || '+91 98765 43210'}</p>
           <p>📧 Email: {state.store?.email || `support@${state.store?.storeName ? state.store.storeName.toLowerCase().replace(/[^a-z0-9]+/g, '-') : 'fashionstore'}.com`}</p>
           <p>🏢 Address: {state.store?.address || 'India Tech Park, Building A, New Delhi'}</p>
        </div>
      </div>
    </div>
  );
