    ...
    <div>
      <section className="custom-hero">
        <div className="custom-hero-text">
          <span className="custom-badge">SportSphere Pro Exclusive</span>
          <h1>English Willow Bats</h1>
          <p>Official match leather footballs, titanium tennis rackets, volleyball nets, and referee whistle packs.</p>
          <button className="custom-btn" onClick={() => state.navigate('category')}>Shop Now ✨</button>
        </div>
        <div className="custom-hero-visual">
          <img src="https://placehold.co/500x400/d1fae5/059669?text=Field%20Equipment" alt="Hero Visual" className="custom-hero-img" />
        </div>
      </section>
        ...
        <div className="custom-category-grid">
          {demoCategories.slice(0, 4).map(cat => (
            <div key={cat.id} className="custom-category-card" onClick={() => { state.setSelectedCategory(cat.name); state.navigate('category'); }}>
              <div className="custom-category-icon" style={{ background: '#d1fae5' }}>{cat.icon}</div>
              <h3>{cat.name}</h3>
            </div>
          ))}
        </div>
