    ...
    <div>
      <section className="custom-hero">
        <div className="custom-hero-text">
          <span className="custom-badge">TimberCraft Rustic Exclusive</span>
          <h1>Solid Oak Cabinets</h1>
          <p>Handcrafted solid teak tables, oak bookshelves, raw wooden stools, and rustic dining benches.</p>
          <button className="custom-btn" onClick={() => state.navigate('category')}>Shop Now ✨</button>
        </div>
        <div className="custom-hero-visual">
          <img src="https://placehold.co/500x400/f5e6e1/78350f?text=Timber%20Oak%20Table" alt="Hero Visual" className="custom-hero-img" />
        </div>
      </section>
        ...
        <div className="custom-category-grid">
          {demoCategories.slice(0, 4).map(cat => (
            <div key={cat.id} className="custom-category-card" onClick={() => { state.setSelectedCategory(cat.name); state.navigate('category'); }}>
              <div className="custom-category-icon" style={{ background: '#f5e6e1' }}>{cat.icon}</div>
              <h3>{cat.name}</h3>
            </div>
          ))}
        </div>
