    ...
    <div>
      <section className="custom-hero">
        <div className="custom-hero-text">
          <span className="custom-badge">GizmoHub Gadgets Exclusive</span>
          <h1>Living in the Future</h1>
          <p>Smart digital key organizers, pocket-sized lasers, tracking tags, and noise dampeners.</p>
          <button className="custom-btn" onClick={() => state.navigate('category')}>Shop Now ✨</button>
        </div>
        <div className="custom-hero-visual">
          <img src="https://placehold.co/500x400/e8f5e9/4caf50?text=Smart%20Gadgets" alt="Hero Visual" className="custom-hero-img" />
        </div>
      </section>
        ...
        <div className="custom-category-grid">
          {demoCategories.slice(0, 4).map(cat => (
            <div key={cat.id} className="custom-category-card" onClick={() => { state.setSelectedCategory(cat.name); state.navigate('category'); }}>
              <div className="custom-category-icon" style={{ background: '#e8f5e9' }}>{cat.icon}</div>
              <h3>{cat.name}</h3>
            </div>
          ))}
        </div>
