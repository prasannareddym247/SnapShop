  ...
  return (
    <div className="custom-page-container">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem', marginTop: '2rem' }}>
        <div>
          <img src={product.image || 'https://placehold.co/400x400'} alt={product.name} style={{ width: '100%', borderRadius: '24px', border: '1px solid var(--border-color)' }} />
        </div>
        <div>
          <h2>{product.name}</h2>
          <p style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--primary-color)' }}>₹{product.discountedPrice || product.price}</p>
          {product.originalPrice && product.discountedPrice && product.originalPrice !== product.discountedPrice && (
            <p style={{ textDecoration: 'line-through', color: 'var(--text-muted)' }}>₹{product.originalPrice}</p>
          )}
